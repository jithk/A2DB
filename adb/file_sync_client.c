...
#define  JVK_ENABLE_PROGRESS

#ifdef JVK_ENABLE_PROGRESS
...
static void show_progress(int percentageComplete,const char * filename)
{


    if(filename != NULL)
        fprintf(stdout,"\r %-10s [",(char*)basename(filename));
    else
        fprintf(stdout,"\r [");
    int maxLen = 50; /* max length of progress bar*/
    int i = 0,max =(percentageComplete*maxLen)/100 ;
    ...
    fprintf(stdout,"] [ %d%% ]",percentageComplete);
    fflush(stdout);
}
#endif
...
#ifdef JVK_ENABLE_PROGRESS
    long size = 0;
    unsigned long long completedTransefer = 0;
#endif
